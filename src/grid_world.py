...
class GridWorld():
    ...
    def add_policy(self, policy_matrix):
        ...
        for patch in self.ax.patches:
            ...
            if not (isinstance(patch, patches.Polygon) or isinstance(patch, patches.Circle)):
                continue

            # 获取 facecolor 和 edgecolor（只比较前三个通道，忽略透明度）
            fc = patch.get_facecolor()[:3] if len(patch.get_facecolor()) >= 3 else patch.get_facecolor()
            ec = patch.get_edgecolor()[:3] if len(patch.get_edgecolor()) >= 3 else patch.get_edgecolor()

            target_color = self.color_policy[:3]  # 只保留 RGB 三通道

            if (np.allclose(fc, target_color, atol=1e-2) or
                np.allclose(ec, target_color, atol=1e-2)):
                patch.remove()
                removed_count += 1
